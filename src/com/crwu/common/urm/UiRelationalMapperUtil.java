...
import com.crwu.utils.DateUtil;
import com.crwu.utils.ReflectUtil;
/**
 * @author cr.wu
 * 
 *         2015年8月25日
 ...
 *  urm类，ui和对象映射
 */
public class UiRelationalMapperUtil {
	public static final int OBJECT_TO_UI = 1;
	public static final int UI_TO_OBJECT = 2;
	
	public static void object2Ui(Object model, Object swt) {
		modelSwtChange(model , swt , OBJECT_TO_UI);
	}
	
	public static void ui2Object(Object model, Object swt) {
		modelSwtChange(model , swt , UI_TO_OBJECT);
	}
...
//					xxx.log(ReflectUtil.class, "swt的属性:    " + tdF.getName()+"  不能找到");
				...
				if (type == OBJECT_TO_UI) {
					UiRelationalMapperUtil.modelToSwt(swtCurrentF, swt, tdF, model);
				} else {
					UiRelationalMapperUtil.swtToModel(swtCurrentF, swt, tdF, model);
				}
	...
	public static void modelToSwt(Field swtF, Object swt, Field tdF, Object model) throws Exception {
		...
		if (swtF.get(swt) instanceof Text) {
			...
			tt.setSelection(b);
		}else if(swtF.get(swt) instanceof UiObjectChange){
			UiObjectChange change = (UiObjectChange) swtF.get(swt);
			change.Objecst2Ui(tdF.get(model));
...
//			if (swtF.getType().getSimpleName().equals("Text")||swtF.getType().getSimpleName().equals("YtText")||swtF.getType().getSimpleName().equals("YtVerfityText")) {
			...
			UiObjectChange change = (UiObjectChange) swtF.get(swt);
			ReflectUtil.setValue(model, tdF, change.ui2Object());
		}
	}
}
