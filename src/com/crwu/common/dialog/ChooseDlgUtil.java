...
public class ChooseDlgUtil {
	 ...
	 * @param fileter 过滤器
	 * **/
	public static String openFile(String path,String[] filter){
		...
		if(add){
			file += filter[0].substring(1, filter[0].length());
		}
		xxx.log(ChooseDlgUtil.class,"选择的文件path:       "+file);
		return file;
	}
	/**
	 * 选择文件
	...
	public static String openDir(String path){
		...
		if (saveFile != null) {
			...
			xxx.log(ChooseDlgUtil.class,directory.getPath());
		}
		return saveFile;
	}
	/**
	...
	public static RGB openColor(){
		...
		xxx.log(ChooseDlgUtil.class,rgb.red + ","+rgb.green+","+rgb.blue);
		xxx.log(ChooseDlgUtil.class,rgb.red + "   "+rgb.green+"   "+rgb.blue);
//		if (rgb != null) {
