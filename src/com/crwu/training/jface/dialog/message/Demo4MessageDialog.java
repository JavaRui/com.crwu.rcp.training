    ...
    private void initButton(Composite composite, final Text console) {
        ...
        createBtn(composite , "信息提示框框" , new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                MessageDialog d = new MessageDialog(Display.getCurrent().getActiveShell(), "这是�?个信息提示对话框标题", image,
                        "这是�?个信息提示对话框，可以改变按钮的设置", MessageDialog.INFORMATION, new String[] { "查看", "保存", "确认" }, 1);
                int i = d.open();
                console.append("\n点击�?   " + i);
            }
        });
        ...
        createBtn(composite , "邮件校验输入�?",new SelectionAdapter(){
            @Override
            public void widgetSelected(SelectionEvent e) {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                IRunnableWithProgress runnable = new IRunnableWithProgress() {
                    ...
                    public void run(IProgressMonitor monitor)
                        ...
                        // TODO Auto-generated method stub
                        monitor.beginTask("�?始执行任�?", 100);
                        for(int i = 0;i<10&&!monitor.isCanceled();i++){
                            Thread.sleep(500);
                            monitor.worked(10);
                            monitor.subTask("已经完成�?   "+i);
                        }
                        monitor.done();
                        if(monitor.isCanceled()){
                            System.err.println("已取�?");
                        }else{
                            System.err.println("已完�?");
        ...
        createBtn(composite, "多行错误提示�?", new SelectionAdapter() {
            @Override
            public void widgetSelected(SelectionEvent e) {
                Status[] status = new Status[2];
                status[0] = new Status(IStatus.ERROR,DUMMY_PLUGIN,1,"未找到该�?",new ClassNotFoundException());
                status[1] = new Status(IStatus.ERROR,DUMMY_PLUGIN,1,"未找到该�?",new FileNotFoundException());
//              Status status = new Status(IStatus.ERROR,dummyPlugin,1,"未找到该�?",new ClassNotFoundException());
//              ErrorDialog d = new ErrorDialog(Display.getCurrent().getActiveShell(), "提示错误", "装载错误", status, IStatus.ERROR);
                MultiStatus mu = new MultiStatus(DUMMY_PLUGIN, IStatus.OK, status, "发现�?大堆错误",new Exception());
